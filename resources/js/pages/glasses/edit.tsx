...
export default function EditGlasses({ glasses }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: 'Edit',
            href: `/glasses/${glasses.id}/edit`,
        },
    ];

    const { data, setData, put, processing, errors } = useForm({
        model_name: glasses.model_name,
        brand: glasses.brand,
        purchase_price: glasses.purchase_price.toString(),
        selling_price: glasses.selling_price.toString(),
        stock_quantity: glasses.stock_quantity.toString(),
        low_stock_threshold: glasses.low_stock_threshold.toString(),
        description: glasses.description || '',
        frame_type: glasses.frame_type || '',
        frame_material: glasses.frame_material || '',
        lens_type: glasses.lens_type || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('glasses.update', glasses.id));
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit ${glasses.brand} ${glasses.model_name} - GlassesFlow`} />
            
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-6">
                {/* Header */}
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
                        ✏️ Edit {glasses.brand} {glasses.model_name}
                    </h1>
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <p className="text-sm text-gray-500 mt-1">Current: {glasses.stock_quantity}</p>
                            </div>
                        ...
                        <div className="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <Button type="submit" disabled={processing}>
                                {processing ? '💾 Updating...' : '💾 Update Glasses'}
                            </Button>
                        </div>
                    </form>
                </div>
